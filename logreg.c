...
static struct argp argp = {
    options, parse_opt, "FILE", "Logistic Regression example", NULL, NULL, NULL,
};
  ...
  for (size_t iter = 0; iter < iterations; iter++) {
    for (size_t j = 0; j < train_dataset->n; j++) {
      ...
      for (size_t i = 0; i < train_dataset->m; i++) {
        ...
        int y_i = (int)example[train_dataset->n - 1];
        if (j == 0)
          sum += (h_i - y_i) * 1;
        else
    ...
    for (size_t i = 0; i < train_dataset->m; i++) {
      ...
      if (y_i == 1) {
        ...
        train_cost += -log(1 - h_i);
      }
    }
    train_cost /= train_dataset->m;
